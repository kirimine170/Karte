...
func TestFiniteFallbackPDFConfigDimensionsUsesPaperRectForFiniteMode(t *testing.T) {
	widthPt, heightPt := finiteFallbackPDFConfigDimensions(595.0, 842.0)
	if widthPt != 595.0 || heightPt != 842.0 {
		t.Fatalf("finite fallback should use paper rect, got %.1fpt x %.1fpt", widthPt, heightPt)
	}
}
