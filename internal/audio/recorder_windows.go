...
func NewRecorder() (*Recorder, error) {
	return nil, fmt.Errorf("audio recording is not supported on Windows build (PortAudio backend is macOS only)")
}

func (r *Recorder) Start(callback func(samples []float32)) error {
	return fmt.Errorf("audio recording is not supported on Windows build")
}
