...
func tokenize(text string) []string {
	...
	for _, r := range text {
		if unicode.IsLetter(r) || unicode.IsNumber(r) || r >= 0x3040 && r <= 0x309F || r >= 0x30A0 && r <= 0x30FF || r >= 0x4E00 && r <= 0x9FAF {
			// Include letters, numbers, hiragana, katakana, CJK ideographs
			current.WriteRune(r)
		} else {
			if current.Len() > 0 {
