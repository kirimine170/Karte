...
2. **ファイルの編集**
   ...
   - Ephyなどが追加・削除したファイルは，左上の「一覧を更新」で再起動せず反映できます．検索条件・選択中のファイル・未保存の編集内容は保持します．本文を読み直す場合は，一覧から対象ファイルを選び直してください．
   - 一覧の更新処理は`Sidebar.refreshFileList()`に集約しています．今回は明示ボタンと既存のapp内イベントから利用し，Ephyからの自動呼出しや外部APIは今後の拡張です．
